...
class LiabilityAgent(BaseAgent):
    ...
    def process(self, message: A2AMessage, db: Session) -> A2AMessage:
        ...
        try:
            ...
            result = self._assess_liability(diagnosis, product)
            output = {
                "case_id": case_id,
                "diagnosis": diagnosis,
                "product": product,
                "liability": result["conclusion"],
                "liability_detail": result["detail"],
                "exclusions_checked": result["exclusions"],
                "waiting_period_met": True,
            }

            self.complete_trace(db, trace_id, output, confidence=result["confidence"])
            return A2AMessage(
