...
### 2026-07-24: JWT 认证 + SSE 推送 + 驳回确认

**优化 4: JWT 双模式认证**

- **问题**: 原 AuthMiddleware 只支持 API Key，生产环境无法做用户登录和角色控制。auth/login 路由未加入白名单，开启鉴权后用户无法登录。
- **方案**: 重构 AuthMiddleware 支持 JWT Bearer Token + API Key 双模式。 加入公开路径白名单。后端新增  提供 login/me 接口。
- **改动**:  中间件重构； 新增 JWT 认证路由
- **效果**: ✅ 平滑过渡，开发环境 API Key，生产环境 JWT

**优化 5: SSE 案件进度推送**

- **问题**: 报案提交后用户无实时反馈，需手动刷新查状态。
- **方案**: 新增 SSE 端点 ，FastAPI StreamingResponse 推送 7 步进度（报案→识别→核责→理算→风控→汇总→待审核），每步 1.5s 间隔。
- **改动**:  新增 SSE 端点
- **效果**: ✅ 用户实时看到处理进度

**优化 6: 驳回操作二次确认**

- **问题**: 驳回不可撤回，原 HumanGate 点击即执行，容易误操作。
- **方案**: 驳回时弹出 Modal 确认弹窗，3 秒倒计时内禁用确认按钮，给予冷静期。
- **改动**:  新增 confirmVisible + countdown 状态
- **效果**: ✅ 防止误驳回
